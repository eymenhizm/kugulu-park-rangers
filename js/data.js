...
function mergeKPRData(remote) {
  const defaults = JSON.parse(JSON.stringify(window.DEFAULT_KPR_DATA));
  if (!remote || typeof remote !== "object") return defaults;

  return {
    ...defaults,
    ...remote,
    club: { ...defaults.club, ...(remote.club || {}) },
    players: Array.isArray(remote.players) && remote.players.length > 0 ? remote.players : defaults.players,
    news: Array.isArray(remote.news) && remote.news.length > 0 ? remote.news : defaults.news,
    fixtures: Array.isArray(remote.fixtures) && remote.fixtures.length > 0 ? remote.fixtures : defaults.fixtures,
    achievements: Array.isArray(remote.achievements) && remote.achievements.length > 0 ? remote.achievements : defaults.achievements,
    league: { ...defaults.league, ...(remote.league || {}) }
  };
}
...
async function supabaseRequest(method = "GET", body = null) {
  ...
  const response = await fetch(SUPABASE_URL + "/rest/v1/site_data?id=eq.1", options);
  ...
  async load() {
    try {
      ...
      if (Array.isArray(rows) && rows.length > 0 && rows[0].data) {
        return mergeKPRData(rows[0].data);
      }
    } catch (error) {
      console.error("Supabase verisi okunamadı:", error);
    }
    return JSON.parse(JSON.stringify(window.DEFAULT_KPR_DATA));
  },

  async save(data) {
    try {
      await supabaseRequest("PATCH", { data: data });
