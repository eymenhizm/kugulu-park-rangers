document.addEventListener("DOMContentLoaded",()=>{
  const root=document.querySelector("#admin-app"); let data=KPR.load();
  ...
  function render(){
    root.innerHTML=`
      ...
      <div class="savebar"><button id="save">DEĞİŞİKLİKLERİ KAYDET</button><button id="reset">VARSAYILANLARA DÖN</button><span id="saved"></span></div>`;
    document.querySelector("#save").onclick=save;
    document.querySelector("#reset").onclick=()=>{if(confirm("Tüm yerel değişiklikler silinsin mi?")){localStorage.removeItem("kpr-data");data=KPR.load();render();}};
  }
  function save(){
    ...
    KPR.save(data); document.querySelector("#saved").textContent="✓ Kaydedildi";
  }
  render();
});